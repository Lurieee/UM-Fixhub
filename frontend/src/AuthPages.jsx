...
export function AdminLoginPage({ onNavigate }) {
  ...
  const submit = (event) => {
    event.preventDefault();
    if (username === "UMAdmin" && password === "UMadmin@123") onNavigate("/admin");
    else setError("Use the provided administrator credentials to continue.");
  };
  return <div className="auth-page"><section className="auth-art"><Brand /><div className="art-copy"><h1>Facilities<br />administration.</h1><p>Secure access for the UM FixHub facilities team to manage reports, staff, analytics, and system settings.</p></div></section><section className="auth-content"><form onSubmit={submit} className="auth-form"><button type="button" className="back" onClick={() => onNavigate("/login")}><ArrowLeft />Back to Student Login</button><h1>Admin Login</h1><p className="intro">Sign in with your authorized facilities administrator account.</p><label className="auth-field"><span>Administrator Username</span><div><UserRound className="h-4 w-4" /><input value={username} onChange={(event) => setUsername(event.target.value)} placeholder="UMAdmin" autoComplete="username" /></div></label><label className="auth-field"><span>Password</span><div><LockKeyhole className="h-4 w-4" /><input value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete="current-password" /></div></label>{error && <p className="auth-error">{error}</p>}<button className="auth-submit">Log In to Admin Panel</button><p className="switch">Need a student account? <button type="button" onClick={() => onNavigate("/signup")}>Create Account</button></p></form></section></div>;
}
