import { ArrowLeft, CheckCircle2, Eye, EyeOff, LockKeyhole, Mail, Menu, ShieldCheck, UserRound, Wrench } from "lucide-react";
import { useEffect, useState } from "react";
import api from "./lib/api";

const Field = ({ label, name, type = "text", icon: Icon, placeholder, value, onChange }) => { const [visible, setVisible] = useState(false); const password = type === "password"; return <label className="auth-field"><span>{label}</span><div><Icon className="h-4 w-4" /><input required name={name} type={password && !visible ? "password" : "text"} placeholder={placeholder} value={value} onChange={onChange} />{password && <button type="button" aria-label="Show password" onClick={() => setVisible(!visible)}>{visible ? <EyeOff /> : <Eye />}</button>}</div></label>; };
const Brand = () => {
...
function TwoFactorStep({ tempToken, onBack, onVerified }) {
  ...
  return <div className="auth-page"><section className="auth-art"><Brand /><div className="art-copy"><h1>One more step<br />to keep you secure.</h1><p>Enter the 6-digit code from your authenticator app to finish signing in.</p></div></section><section className="auth-content"><form onSubmit={submit} className="auth-form"><button type="button" className="back" onClick={onBack}><ArrowLeft />Back</button><h1>Enter your code</h1><p className="intro">Open your authenticator app and enter the current 6-digit code.</p><label className="auth-field"><span>Authentication Code</span><div><ShieldCheck className="h-4 w-4" /><input required value={code} onChange={(e) => setCode(e.target.value)} placeholder="123456" maxLength={6} inputMode="numeric" /></div></label>{error && <p className="auth-error">{error}</p>}<button className="auth-submit" disabled={submitting}>{submitting ? "Verifying..." : "Verify & Log In"}</button></form></section></div>;
}
...
function AuthPanel({ mode, onNavigate, onLogin }) {
  ...
  if (tempToken) {
    return <TwoFactorStep tempToken={tempToken} onBack={() => setTempToken(null)} onVerified={onLogin} />;
  }

  return <div className="auth-page"><section className="auth-art"><Brand /><div className="art-copy"><h1>{signup ? <>Join the community<br />maintaining UM.</> : <>Ensuring excellence<br />in campus facilities.</>}</h1><p>{signup ? "Create your account to start reporting facilities issues. Your contribution helps ensure a better learning environment for all students and faculty." : "Log in to your account to report maintenance issues, track progress, and help us maintain a safe campus environment for everyone."}</p></div></section><section className="auth-content"><form onSubmit={submit} className="auth-form"><button type="button" className="back" onClick={() => onNavigate(signup ? "/login" : "/")}><ArrowLeft />{signup ? "Back to Login" : "Back to Home"}</button><h1>{signup ? "Create your account" : "Welcome back!"}</h1><p className="intro">{signup ? "Please fill in your correct student or faculty credentials to get started." : "Enter your credentials to manage and track your maintenance reports."}</p><Field label="Campus Email Address" name="email" icon={Mail} placeholder="e.g. l.odiong.696969@umindanao.edu.ph" value={email} onChange={(e) => setEmail(e.target.value)} /><Field label="Password" name="password" type="password" icon={LockKeyhole} placeholder={signup ? "Create strong password" : "••••••••••••"} value={password} onChange={(e) => setPassword(e.target.value)} />{error && <p className="auth-error">{error}</p>}{!signup && <button type="button" className="forgot">Forgot Password?</button>}<button className="auth-submit">{signup ? "Create Student Account" : "Log In"}</button>
    <p className="switch">{signup ? "Already have an account? " : "Don't have an account? "}<button type="button" onClick={() => onNavigate(signup ? "/login" : "/signup")}>{signup ? "Log In" : "Sign Up"}</button></p>  </form></section></div>;
}
...
export function SignUpPage({ onNavigate, onSignup }) {
  const [form, setForm] = useState({ fullName: "", email: "", studentId: "", password: "", passwordConfirmation: "" });
  const [error, setError] = useState("");
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setError(messages ? Object.values(messages)[0][0] : "Could not create account.");
    }
  };

  return <div className="auth-page"><section className="auth-art"><Brand /><div className="art-copy"><h1>Join the community<br />maintaining UM.</h1><p>Create your account to start reporting facilities issues and help improve the campus for everyone.</p></div></section><section className="auth-content"><form onSubmit={submit} className="auth-form"><button type="button" className="back" onClick={() => onNavigate("/login")}><ArrowLeft />Back to Login</button><h1>Create your account</h1><p className="intro">Your profile will use the information below.</p><SignupField label="Full Name" name="fullName" icon={UserRound} placeholder="e.g. Lorena Odiong" value={form.fullName} onChange={update} /><SignupField label="Campus Email Address" name="email" type="email" icon={Mail} placeholder="e.g. name@umindanao.edu.ph" value={form.email} onChange={update} /><SignupField label="Student ID" name="studentId" icon={Menu} placeholder="e.g. 2026-001234" value={form.studentId} onChange={update} /><SignupField label="Password" name="password" type="password" icon={LockKeyhole} placeholder="Create strong password" value={form.password} onChange={update} /><SignupField label="Confirm Password" name="passwordConfirmation" type="password" icon={LockKeyhole} placeholder="Re-enter password" value={form.passwordConfirmation} onChange={update} />{error && <p className="auth-error">{error}</p>}<button className="auth-submit">Create Student Account</button></form></section></div>;
}
